...
from src.Dialects import (DialectFromFeatureList, DialectFromVector,
                          IndianDialect)
...
transformations = [
    DialectFromFeatureList(feature_list=[feature]) for feature in features
]
...
invents = [
    [transform.transform(ex) for transform in transformations] for ex in tqdm(gen_e)
]
invents = [
    random.sample([ex for ex in invent if ex != gen_e[i]], 1)[0]
